...
matches_data_preprocessing <- function(data){
  ...
  temp[,`:=`(Total_Score = Home_Score + Away_Score)]
  temp[,`:=`(Result_Total=ifelse(Total_Score>'2.5',1,0))]
...
details_data_preprocessing <- function(data,matches,which_bets=c('ou'),removeOlderThan=30){
